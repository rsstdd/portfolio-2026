...
summary: A Next.js, Flask, and Cloud SQL system that projects harvest volume from a growth rate over a fixed window and a diameter band, with precomputed fruit geometry and geospatial scan points.
role: Sole author
...
A Next.js and React client renders orchard scan points as markers on a Google Map, alongside a scan-date picker, a growth-rate input in cubic millimetres per day, and a diameter range selector bounded at 20 to 120mm. On submit it posts the query to the backend and renders the returned distribution as a histogram with the average fruit size.

The brief asked for two date selectors, for scan date and harvest date. The delivered client takes one and derives the harvest date as thirty days after the scan, which removes an input and an invalid-range state at the cost of a fixed projection window. That is a scope decision rather than an omission, and it is recorded in the repository as one.
...
The estimate applies a single supplied growth rate uniformly over a fixed thirty-day window. It does not account for variety, position in the canopy, or seasonal variation, all of which a real harvest model would need, and the fixed window means the projection cannot be asked a question about any other horizon.
