---
title: Hydro sensor
summary: A Python edge agent that speaks the Atlas Scientific serial protocol over FTDI, temperature-compensates pH and conductivity, and fans telemetry out to independent sinks.
role: Sole author
...
One probe is flagged as the reference. Its temperature reading is pushed to the other probes with a `T,<temp>` command before they are read, because uncompensated pH and conductivity readings drift with water temperature and would otherwise be recorded as real change.
...
| Push a reference temperature before reading pH and EC | Uncompensated readings drift with water temperature, so without this the agent records an artifact of the water rather than a change in the chemistry. |
