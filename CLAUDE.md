<!--
  The project ruleset lives here, not in AGENTS.md.

  AGENTS.md is regenerated by `next dev`: it replaces everything between its
  BEGIN/END nextjs-agent-rules markers. These rules previously sat inside those
  markers, twice, so every dev-server start silently deleted them. Content
  outside the markers survives, which is why AGENTS.md still points here.
--> markers. These rules previously sat inside
  those markers, twice, so every dev-server start silently deleted them. Content
  outside the markers survives, which is why AGENTS.md still points here.
-->
...
- **The CV is exported by rendering the page, and the print stylesheet is the fragile part.** `/cv` renders `content/cv.mdx`, and `scripts/render-cv.mjs` drives that same page through print media in Chromium to write `public/cv/ross-todd.pdf`, so the page and the document cannot disagree. This rule used to end "not exported"; the script is what changed it. What did not change is where the silent breakage lives: the print stylesheet in `components.css`, not a page count: `@media print` and `.no-print` hide the nav, footer, and on-screen-only copy, and nobody notices when that stops working because nobody prints a page they did not change, which is why the script asserts the chrome is hidden and refuses to write a CV with a nav bar in it. The CV is a full CV at roughly 2,400 words and is legitimately several pages. The one-page rule belongs to the ATS-facing resume variants in the Job Search repo, which is also where the incident behind it happened: five of the original ten "finished" resumes were silently 2 pages. Do not apply that assertion here.
