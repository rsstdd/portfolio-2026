...
**Iconography.** Almost every label is a word. The one exception is a control where a word is slower to read than a shape, and today that is the theme switch: sun, moon, and a half-filled circle for follow-the-system, where `LIGHT` and `DARK` were three uppercase words competing with the nav beside them. The set is Lucide geometry at 20px on a 24 grid, 2px stroke, outlined, ink-muted by default and accent when selected, with filled and outlined never mixed. It is hand-authored inline in `src/components/ui/theme-icons.tsx` rather than installed: three glyphs do not justify a dependency, and Lucide has removed brand marks, so it could not supply the filled mark below in any case. The bar for a fourth icon is the same as the bar was for these three, which is that the word must be genuinely worse.

**The one filled mark.** The GitHub mark on source links is the only filled glyph on the site, and it stays an exception to the outlined set above rather than joining it: it is a brand mark, and a brand mark redrawn as a stroke outline stops being recognisable, which is the entire reason it is worth showing instead of the word. That is the distinction the never-mix rule is drawing, and it is why adding a filled icon is a different question from adding an outlined one. Filled marks are closed at one. It is sized in `em` and filled with `currentColor` so it inherits the type size and colour of its link. Where it leads a standalone link, that link takes `link-standalone--no-flag`, because the datum tick and the mark both claim the leading slot and a link may wear only one.
